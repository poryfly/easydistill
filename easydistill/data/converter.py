...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        if ((not self.dataset_attr.ranking or self.dataset_attr.had_output) and len(aligned_messages) % 2 != 0) or (
        ...
        elif not self.dataset_attr.had_output:
            prompt = aligned_messages
            response = [{"role": Role.ASSISTANT.value, "content": ""}]
        else:  # normal example
            prompt = aligned_messages[:-1]
            response = aligned_messages[-1:]
